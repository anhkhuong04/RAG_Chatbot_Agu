...
const Message: React.FC<MessageProps> = ({ message }) => {
  ...
  return (
    <div
      className={`message-animate flex gap-4 mb-8 ${isUser ? "flex-row-reverse justify-start" : "justify-start"
        }`}
    >
      {/* Avatar */}
      ...
      <div className={`min-w-0 ${isUser ? "max-w-[70%]" : "max-w-[90%]"}`}>
        ...
        >
          <span
            className={`text-sm font-bold ${isUser ? "text-gray-700" : "gradient-text-bot"
              }`}
          >
            {isUser ? "Bạn" : "Bot"}
          </span>
