...
class CSVQueryEngine:
    ...
    def __init__(self, structured_dir: str, get_intent_prompt_fn=None):
        """
        Args:
            structured_dir: Path to the structured data directory
    ...
    def _build_instruction(
        ...
        """Build the PandasQueryEngine instruction string."""
        columns_str = ", ".join(df.columns.tolist())
        notes_section = ""
        if dynamic_notes:
            notes_section = (
    ...
    def _validate_pandas_output(raw_output: str, engine) -> bool:
        """
        Validate that PandasQueryEngine output contains all expected columns.
        Returns True if output seems complete, False if columns appear missing.
        """
    ...
    async def _retry_pandas_query(engine, message: str) -> str | None:
        """
        Retry PandasQueryEngine with explicit all-columns instruction.
        Used when first attempt produced output with missing columns.
        """
        retry_message = (
            f"{message}\n\n"
    ...
    async def handle_query(
        ...
        Includes validation + retry if output appears incomplete.

        Args:
            engine: PandasQueryEngine instance
            message: User's question
            source_label: Label for the data source
            intent: Intent key for prompt lookup
...
- BẮT BUỘC giữ nguyên TẤT CẢ các cột dữ liệu có trong kết quả. KHÔNG được bỏ cột nào."""
    ...
    def clear(self):
        """Clear cached engines (called during cache invalidation)."""
        self._diem_chuan_engine = None
        self._hoc_phi_engine = None
